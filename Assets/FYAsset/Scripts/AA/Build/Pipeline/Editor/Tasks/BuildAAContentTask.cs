...
/// </summary>
public class BuildAAContentTask : IBuildTask
{
    ...
    public BuildTaskResult Execute(BuildRunContext ctx)
    {
        ...
        try
        {
            ...
            Debug.Log("[BuildAAContentTask] 开始执行 Addressables BuildPlayerContent...");
    ...
    private static void ConfigureBasicSettings(AddressableAssetSettings settings, BuildRequest request)
    {
        ...
        foreach (var group in settings.groups)
        {
            ...
            if (group.Name == "Built In Data" || group.HasSchema<PlayerDataGroupSchema>())
            {
                ...
                {
                    Debug.LogWarning($"[BuildAAContentTask] 修复冲突：移除 {group.Name} 中错误的 BundledAssetGroupSchema");
        ...
        Debug.Log($"[BuildAAContentTask] Addressables BuildPath 已注入并保存: Catalog={catalogBuildPath}, Remote={remoteBuildPath}");
    }
    ...
    private static void SetSchemaPathToRemote(AddressableAssetSettings settings, BundledAssetGroupSchema schema)
    {
        ...
        if (changed)
            Debug.Log($"[BuildAAContentTask] 已将 Schema 路径修正为 Remote: {schema.Group.Name}");
    }
}
