...
public abstract class HotfixFlowBase
{
    ...
    private string PackageIndexUrl => FYAssetPathUtility.JoinUrl(HotfixUrl, FYAssetSettings.PACKAGE_INDEX_FILE_NAME);
    ...
    public float CurrentProgressValue { get; private set; } = 0f;
    ...
    private void BeginStep(string stepName)
    {
        ...
        _currentStepIndex = Array.IndexOf(StepNames, _currentStepName);
        if (_currentStepIndex < 0)
        {
            Debug.LogError($"[HotfixManager] Unknown hotfix step: {_currentStepName}");
            _currentStepIndex = 0;
        }
    ...
    private void ReportStepProgress(float stepProgress)
    {
        ...
        float overall = Mathf.Clamp01((_currentStepIndex + clamped) / StepNames.Length);
    ...
    /// </summary>
    private async Task<BuildIndexData> StepLoadBuildIndexAsync()
    {
        BeginStep("Load BuildIndex");
    ...
    /// 步骤2：初始化热更后端
    /// </summary>
    private async Task<bool> StepInitializeBackendAsync(IHotfixPipeline pipeline)
    {
        BeginStep("Initialize backend");
    ...
    /// </summary>
    private async Task<bool> StepDownloadPackageIndexAsync(HotfixContext ctx)
    {
        BeginStep("Download PackageIndex");
    ...
    /// 步骤4：加载本地热更版本信息
    /// </summary>
    private async Task<HotfixVersionInfo> StepLoadLocalVersionAsync(IHotfixPipeline pipeline)
    {
        BeginStep("Load local version");
    ...
    /// 步骤5：获取远端热更版本信息
    /// </summary>
    private async Task<HotfixVersionInfo> StepFetchRemoteVersionAsync(IHotfixPipeline pipeline, HotfixContext ctx)
    {
        BeginStep("Fetch remote version");
    ...
    /// </summary>
    private IReadOnlyList<BundleDownloadItem> StepGetBundleDownloadList(IHotfixPipeline pipeline,
        HotfixVersionInfo remoteInfo)
    {
        BeginStep("Prepare download list");
    ...
    /// </summary>
    private async Task<bool> StepDownloadBundlesAsync(
        HotfixContext ctx,
        IReadOnlyList<BundleDownloadItem> remoteBundles,
        HotfixVersionInfo localInfo)
    {
        BeginStep("Download bundles");
    ...
    /// </summary>
    private async Task<bool> StepPostDownloadAsync(IHotfixPipeline pipeline, HotfixContext ctx)
    {
        BeginStep("Post download");
    ...
    /// </summary>
    private void StepApplyUpdate(HotfixContext ctx, HotfixVersionInfo remoteVersionInfo)
    {
        BeginStep("Apply update");
    ...
    private bool IsPackageIndexBackendTrusted(PackageIndex packageIndex, out string error)
    {
        string expectedBackend = BackendModeName;
        ...
        if (!string.Equals(packageIndex.BackendMode, BackendModeName, StringComparison.OrdinalIgnoreCase))
        {
    ...
    private async Task<bool> DownloadBundleWithRetry(
        ...
        Action onDone)
    {
        int maxRetries = Mathf.Max(0, HotfixMaxRetryCount);
        int totalAttempts = maxRetries + 1;
        float baseDelaySeconds = Mathf.Max(0f, HotfixRetryBaseDelaySeconds);
    ...
    /// </summary>
    private bool VerifyBundleCRC(string path, BundleDownloadItem bundleInfo)
    {
    ...
    /// <summary>
    /// 热更流程收尾：触发回调，初始化 concrete package manager 和 LuaEnv
    /// </summary>
    protected abstract Task FinishHotfix();
