...
> Purpose: Reference for XLuaHotfix build-time asset collection and indexing study
...
### What to adopt:
- **Three-rule interface pattern** (IAddressRule / IPackRule / IFilterRule) maps well to a project-side collector framework
...
- **Sub-directory collector**: YooAsset uses PackTopDirectory as a pack rule; a project-side collector may need a dedicated sub-directory collector concept
- **Tag system**: Our RuntimeAssetEntry.Labels maps to YooAsset's Tags but with case-insensitive matching (under the current runtime asset-entry design)
...
- **RawFile-specific rules** - deferred to a later dedicated runtime/build step
