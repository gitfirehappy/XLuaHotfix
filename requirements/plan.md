...
| E5-1 | Build pipeline core engine — IBuildTask + BuildContext + BuildTaskResult + BuildPipelineConfig SO + BuildTaskResolver + DAGScheduler (Kahn topology + deterministic execution + dependency/cycle validation + staged WriteKeys semantics + Read-before-Write warning + ValidatePair/ValidateAll). Note: "batch" means logically-independent tasks grouped by topological level, executed sequentially on main thread (Unity Editor single-threaded). 8 new files, 608 lines. **2026-05-07 review fixes: DAGScheduler Read-before-Write unsound + BuildTaskResolver duplicate TaskName fail-fast**; **2026-06-01 correction: WriteKeys are write/update declarations, not exclusive write locks.** **2026-06-05 correction: `SequentialMode` removed because execution is already deterministic serial on Unity main thread.** | **Realized** (plan-E5-1.md) |
