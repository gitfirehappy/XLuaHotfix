...
- `AssetCollectionSettingPanel` is a UI Toolkit panel that edits `AssetCollectionSetting` with package/group navigation, group bundle packing mode, and collector editing.
- `CollectorPanel` is a UI Toolkit panel focused on the current group collector list plus validation and scan preview. Its scan preview can select an asset and edit the GUID-keyed AssetEntry metadata in the right-side details pane.
...
- `TaskCollectAssets` is the backbone scan task. It loads `AssetCollectionSetting`, runs `CollectionScanner.Scan()`, writes `CollectedAssets` and `SharePolicies` into `BuildContext`, and is the dependency source for dependency analysis and builtin collection.
...
- `SettingsPanel`, `VersionPanel`, `AAConfigPanel`, `AABuildPanel`, `AAReportPanel`, `AssetCollectionSettingPanel`, `CollectorPanel`, `PipelinePanel`, `BuilderPanel`, and generic `PlaceholderPanel` all expose UI Toolkit content through `IBuildPipelinePanel.CreateContent()`.
