...
public static class AAHotfixGroupMover
{
    ...
    public static bool Apply(FileDiff diff)
    {
        ...
        if (HasPendingMoves)
        {
            Debug.LogError("[AAHotfixGroupMover] 存在待处理的 Hotfix Group 移动记录。准备下一次 Hotfix 前请运行 ResetGroupsToOriginal。");
            return false;
        }
        ...
        if (settings == null)
        {
            Debug.LogError("[AAHotfixGroupMover] AddressableAssetSettings 为 null。");
            return false;
        }
        ...
        Debug.Log($"[AAHotfixGroupMover] 已将 {undoLog.Entries.Count} 个 asset 移入 {FYAssetSettings.HOTFIX_GROUP_NAME}。");
        return true;
    }
    ...
    public static HotfixGroupRestoreResult Restore()
    {
        ...
        if (undoLog.Entries.Count == 0)
        {
            Debug.Log("[AAHotfixGroupMover] 没有待恢复的 Hotfix Group 移动记录。");
        ...
        for (int i = 0; i < undoLog.Entries.Count; i++)
        {
                    ...
                    out bool fallsBackToDefault, out string reason))
            {
                ...
                Debug.LogWarning($"[AAHotfixGroupMover] 恢复已暂缓：{reason}");
                continue;
            }
    ...
    private static void MoveArtifacts(List<FileDigest> artifacts, AddressableAssetSettings settings, AddressableAssetGroup hotfixGroup, HotfixGroupUndoLog undoLog)
    {
        for (int i = 0; i < artifacts.Count; i++)
        {
            ...
            if (entry == null)
            {
                Debug.LogWarning($"[AAHotfixGroupMover] 未找到 guid 对应的 Addressable entry：{artifact.Name}");
                continue;
            }
    ...
    private static HotfixGroupUndoLog LoadUndoLog()
    {
        ...
        {
            Debug.LogWarning($"[AAHotfixGroupMover] 读取 undo log 失败：{ex.Message}");
            return new HotfixGroupUndoLog();
        }
    }
    ...
    public static void RunSelfCheck()
    {
        string root = Path.Combine(
            Path.GetTempPath(),
            nameof(AAHotfixGroupMover) + "_" + Guid.NewGuid().ToString("N"));
        try
        {
            RunUndoLogPersistenceSelfCheck(root);
            Debug.Log("[AAHotfixGroupMover] PASS - undo log 持久化验证通过。");
        }
        finally
        {
