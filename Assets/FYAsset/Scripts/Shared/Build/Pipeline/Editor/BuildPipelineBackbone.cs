...
/// 只提供默认配置创建、主干识别和顺序列表排序；不会在加载或构建时自动修改已有配置资产。
/// </summary>
public static class BuildPipelineBackbone
{
    /// <summary>AB 主干 Task 顺序列表，同时定义编辑器展示顺序。</summary>
    ...
    /// <summary>AA 主干 Task 顺序列表，同时定义编辑器展示顺序。</summary>
    ...
    {
        return CreateTasks(ABTaskNames);
    }
    ...
    {
        return CreateTasks(AATaskNames);
    }
    ...
    private static List<TaskEntry> CreateTasks(string[] taskNames)
    {
