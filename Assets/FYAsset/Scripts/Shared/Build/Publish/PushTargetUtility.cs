...
{
    public static IPushTarget Create(PushTargetConfig config, Func<PushTargetConfig, IPushTarget> extendedFactory = null)
    {
        if (config == null)
        ...
        // 扩展 target 类型（如 CloudflarePages）属 Compat 部署胶水，由调用方注入创建器。
        IPushTarget extendedTarget = extendedFactory?.Invoke(config);
        if (extendedTarget != null)
            return extendedTarget;
        throw new ArgumentOutOfRangeException(nameof(config.Type), config.Type,
            "Unsupported push target type (extended types require an injected factory, e.g. via CompatPushTargetFactory).");
    }
