...
- `AB PIPELINE` contains `AB Config`, `Collector`, and `Pipeline`.
- `AssetCollectionSettingInspector` is a UI Toolkit shortcut inspector that opens `BuildPipelineWindow` directly.
- `CollectorPanel` is the single UI Toolkit panel for `AssetCollectionSetting`. It edits Package share policy, Group labels and bundle packing mode, Collector rows, validation, scan preview, and GUID-keyed AssetEntry metadata in one place.
...
- `SettingsPanel`, `VersionPanel`, `AAConfigPanel`, `AABuildPanel`, `AAReportPanel`, `CollectorPanel`, `PipelinePanel`, `BuilderPanel`, and generic `PlaceholderPanel` all expose UI Toolkit content through `IBuildPipelinePanel.CreateContent()`.
