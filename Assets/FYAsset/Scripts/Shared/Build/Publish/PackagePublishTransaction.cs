...
{
    private readonly BuildBaseline _release;
    ...
    public PackagePublishTransaction(BuildBaseline release, string sourcePackage, string backendRoot)
    {
        ...
        _workRoot = FYAssetPathUtility.JoinFilePath(
            _backendRoot,
            ".fyasset_push",
            _release.PackageName + "_" + Guid.NewGuid().ToString("N").Substring(0, 8));
        _stagedPackage = FYAssetPathUtility.JoinFilePath(_workRoot, "staged", _release.PackageName);
        _backupPackage = FYAssetPathUtility.JoinFilePath(_workRoot, "backup", _release.PackageName);
    ...
    public void Apply()
    {
        ...
        try
        {
            ...
            if (!_sourceAlreadyAtTarget)
            {
                ...
                ValidatePackage(_stagedPackage, _release);
    ...
    internal static void ValidatePackage(string packageDir, BuildBaseline release)
    {
        if (!FileHelper.DirectoryExists(packageDir))
            throw new DirectoryNotFoundException($"Package directory missing: {packageDir}");

        // manifest 文件名随基线走（后端交付时注入 RequiredManifestFileNames），此处不感知后端。
        if (release == null || release.ManifestFileNames == null || release.ManifestFileNames.Count == 0)
            throw new InvalidOperationException("baseline 缺少 ManifestFileNames，无法校验包完整性。");
        foreach (string manifestName in release.ManifestFileNames)
        {
            string manifestPath = FYAssetPathUtility.JoinFilePath(packageDir, manifestName);
            if (!FileHelper.Exists(manifestPath))
                throw new FileNotFoundException($"Package manifest missing: {manifestName}", manifestPath);
        }
    }
    ...
    private void WritePackageIndex()
    {
        ...
        {
            LatestPackage = _release.PackageName,
            LatestVersion = _release.Version,
            BackendMode = _release.BackendMode
        };
