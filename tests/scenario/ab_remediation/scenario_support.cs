...
internal static class Program
{
    private static int Main()
    {
        ...
        {
            ("SummaryOwnershipContract", SummaryOwnershipTests.Run),
            ("SummaryTransactionContract", SummaryTransactionTests.Run),
            ("PackagePurityContract", PackagePurityTests.Run),
            ("PhysicalNamingContract", PhysicalNamingTests.Run),
            ("HotfixCandidateContract", HotfixCandidateTests.Run),
            ("HotfixDeliveryContract", HotfixDeliveryTests.Run)
        };
        ...
        Console.WriteLine($"ab remediation gates: {gates.Length - failures}/{gates.Length} passed");
...
internal static class RepoSource
{
    ...
    /// <summary>
    /// 递归拼接目录下全部 .cs 的**原文**。路径字面量与序列化键被净化流程替换为空引号对，
    /// 因此字面量断言必须使用原文；符号断言使用 <see cref="ReadCode"/> / <see cref="Sanitize"/>。
    /// </summary>
    public static string RawTreeText(string relativeDir)
    {
        var builder = new System.Text.StringBuilder();
        foreach (string file in CsFiles(relativeDir))
        {
            builder.Append('\n');
            builder.Append(File.ReadAllText(Absolute(file)));
        }
        return builder.ToString();
    }

    /// <summary>返回目录树中命中指定符号的文件列表，供失败信息定位。</summary>
    public static List<string> FilesContaining(string relativeDir, string symbol)
    {
        var hits = new List<string>();
        foreach (string file in CsFiles(relativeDir))
        {
            if (HasToken(Sanitize(File.ReadAllText(Absolute(file))), symbol))
                hits.Add(file);
        }
        return hits;
    }
...
/// <summary>
/// 子契约级执行器：每个子契约独立执行并单独报告 GREEN/RED，最后统一汇总结论。
/// </summary>
    ...
    public static void RunAll(params (string Name, Action Run)[] checks)
    {
        ...
        if (failures.Count > 0)
            throw new InvalidOperationException(
                $"{failures.Count}/{checks.Length} 个子契约未满足 -> {string.Join(" | ", failures)}");
    }
}
...
internal static class GateAssert
{
    ...
    public static void Contains(string source, string value, string message) =>
        True(source.IndexOf(value, StringComparison.Ordinal) >= 0, $"{message}（未找到 '{value}'）");

    public static void NotContains(string source, string value, string message) =>
        True(source.IndexOf(value, StringComparison.Ordinal) < 0, $"{message}（仍存在 '{value}'）");
