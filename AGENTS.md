...
- Treat this file as collaboration guidance only. Verify concrete architecture, file paths, and current behavior from code
  and `context/`.
...
- `context/`: AI-facing verified knowledge, English only. No TODOs, plan status, approval text, or next-step workflow.
...
- Prefer the current project-approved resource loading facade in new runtime code; verify the current facade from code and
  `context/`.
