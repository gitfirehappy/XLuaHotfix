...
- `AssetsCollectionPanel` is the single UI Toolkit panel for `AssetCollectionSetting`. It keeps the package/group navigation sidebar, edits group bundle packing mode and collector rows, shows the right-side Package -> Group -> Asset scan tree, and edits GUID-keyed AssetEntry metadata in the same details area.
...
- `SettingsPanel`, `VersionPanel`, `AAConfigPanel`, `AABuildPanel`, `AAReportPanel`, `AssetsCollectionPanel`, `PipelinePanel`, `BuilderPanel`, and generic `PlaceholderPanel` all expose UI Toolkit content through `IBuildPipelinePanel.CreateContent()`.
